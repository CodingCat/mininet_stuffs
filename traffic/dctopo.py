...
class FatTreeTopo(StructuredTopo):
    ...
    def port(self, src, dst):
        ...
        if src_layer == LAYER_HOST and dst_layer == LAYER_EDGE:
            ...
            #print ("src layer is EDGE, dst layer is AGG")
            src_port = dst_id.sw #(dst_id.sw - self.k / 2) #* 2
            dst_port = src_id.sw #* 2 + 1
        elif src_layer == LAYER_AGG and dst_layer == LAYER_CORE:
            src_port = self.k / 2 + dst_id.host - 1#(dst_id.host - 1) * 2
            dst_port = src_id.pod
        elif src_layer == LAYER_CORE and dst_layer == LAYER_AGG:
            src_port = dst_id.pod
            dst_port = self.k / 2 + src_id.host - 1#(src_id.host - 1) * 2
